...
Composite PK `(PoiId, PoiCollectionId)`; two cascade FKs. Many-to-many between Poi and PoiCollection. **Trip fields:** `OrderIndex` (int, 1-based Stop Order; 0 = "not a Stop", e.g. unplaceable); `DwellMinutes` (int?, per-membership dwell so the same POI carries different dwell across trips); and — `AddOutgoingTravelMode` migration — **`OutgoingTravelMode`** (string?, ≤20, one of `TravelMode.All`; **null ≡ AnyAir**, one state — TRIP-LEGMODE-01) the mode of the leg **leaving** this stop, check-constrained by `CK_PoiCollectionItem_OutgoingTravelMode`. Mutated **only** inside `TripOrderingService` (the order/reset path, the per-leg `SetOutgoingTravelModeAsync`, and the bulk `SetAllOutgoingTravelModesAsync` — TRIP-BULKMODE-01, one gated transaction across every leg's From-stop); no schema change for the bulk path.

### RouteSegment (trip leg / distance-matrix cache)
Composite PK `(FromPoiId, ToPoiId, TravelMode)` — **directional** (`TRIP-CACHE-01`: A→B and B→A are distinct rows). Columns: `DurationSeconds` (int, canonical **seconds**), `DistanceMeters` (double, canonical **meters**), `GeometryPolyline` (string?, null = no road geometry), `Fidelity` (string ≤20, check-constrained), `Source` (≤100), `ComputedAt` (UTC), `Version` (`[ConcurrencyCheck]`). Cascade FKs to `Pois`; indexes on `FromPoiId` and `ToPoiId`. `Source` records the provenance provider (`Mock`, `Manual`, `Valhalla` for measured routing, `EstimatedFallback`); the default `MockTravelTimeProvider` emits smart-haversine estimates while the opt-in `ValhallaTravelTimeProvider` writes `Source="Valhalla"`. Cache rows are derived/disposable — invalidated on coord/mode/provider/assumed-speed change; never the source of truth for trip intent. A one-time idempotent startup step (`StartupCleanupService.PurgeOsrmCacheRowsAsync`) deletes stale non-Manual rows with legacy `Source="OSRM"` so the active provider recomputes them (no schema change — still 13 app migrations).
