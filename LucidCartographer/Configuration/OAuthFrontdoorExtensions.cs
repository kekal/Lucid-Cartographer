        ...
        IHostEnvironment environment)
    {
        ...
        services.AddOpenIddict()
            ...
            .AddServer(options =>
            {
                ...
                // with invalid_target). The remote-MCP connector sends the /mcp
                // endpoint URL, while Claude chat sends the bare server root —
                // register both. Validation is ordinal: bare issuer normalizes to
                // "<issuer>/" (Uri adds trailing slash, matching the chat client)
                // while /mcp URL stays verbatim. (Comparison is against Options.Resources.)
