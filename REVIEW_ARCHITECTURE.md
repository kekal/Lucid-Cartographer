...
### CRIT-01: ~~Tailwind CSS loaded from CDN play script -- production app ships a development-only tool~~ RESOLVED
**File:** `Components/App.razor`
**Status:** Fixed. The Tailwind CDN play script has been replaced with a pre-built CSS file (`wwwroot/css/tailwind.css`) generated by the standalone Tailwind CLI. The `tailwind.config.js` at project root contains the custom theme (colors, fonts). The Dockerfile downloads the standalone Tailwind CLI binary and runs the build step before `dotnet publish`, requiring no Node.js. The input file is `wwwroot/css/input.css` and the generated output is `wwwroot/css/tailwind.css` (gitignored as a build artifact).
